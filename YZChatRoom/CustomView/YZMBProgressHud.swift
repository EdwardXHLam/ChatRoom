//
//  YZMBProgressHud.swift
//  YZChatRoom
//
//  Created by YZ-LXH on 2021/3/29.
//
...
class YZMBProgressHud {
    ...
    private class func createHud(view : UIView? = UIApplication.shared.keyWindow, isMask : Bool = false ,isTranslucent: Bool = false) -> MBProgressHUD? {
        ...
        let HUD = MBProgressHUD.showAdded(to: supview
            , animated: true)
        let top = isTranslucent ? 0 : YZScreenInfo.navigationHeight
        HUD.frame = CGRect(x: 0, y: top, width: YZScreenInfo.Width, height: YZScreenInfo.Height - top)
    ...
    private class func createHud(navigationController : UINavigationController?, isMask : Bool = false) -> MBProgressHUD? {
        ...
        let HUD = MBProgressHUD.showAdded(to: supview
            , animated: true)
        HUD.frame = CGRect(x: 0, y: 0, width: YZScreenInfo.Width, height: YZScreenInfo.Height)
        HUD.animationType = .zoom
        if isMask {
            /// 蒙层type,背景半透明.
            HUD.backgroundView.color = UIColor(white: 0.0, alpha: 0.4)
...
extension YZMBProgressHud {
    ...
    ///   - afterDelay: view消失时间
    ///   - isTranslucent: 自己项目中navigationBar是否参与frame设置,默认为False,如果项目中主动将其设置为true,可以传入True.不传的话中弹出view会有半个navigationBar.height的向下偏移.
    class func showTextHudTips(message : String?, view : UIView? = UIApplication.shared.keyWindow , isMask : Bool = true,afterDelay:Double = hudShowTime,isTranslucent: Bool = false) {
        let HUD = self.createHud(view: view, isMask: isMask, isTranslucent: isTranslucent)
        HUD?.mode = .text
